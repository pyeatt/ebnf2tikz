...
/**
 * @file parser.yy
 * @brief Main Bison grammar for parsing EBNF input into the AST.
 *
 * Defines the grammar rules that transform tokenized EBNF input
 * (from lexer.ll) into an AST composed of SequenceNode, ChoiceNode,
 * OptionalNode, LoopNode, TerminalNode, NonterminalNode, and
 * NewlineNode.  After parsing, the grammar action invokes the
 * full compiler pipeline: subsumption, optimization, auto-wrap,
 * and either AST dump or TikZ output.
 *
 * @see lexer.ll for the corresponding Flex tokenizer
 * @see annot_parser.yy for the annotation sub-parser
 * @see driver.hh for the parser driver class
 */
...
grammar : productions {
     ...
     if(drv.get_dump_before())
       {
         if(drv.get_dump_after())
