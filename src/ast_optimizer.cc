...
/**
 * @file ast_optimizer.cc
 * @brief Implementation of iterative fixed-point AST optimization passes.
 *
 * The optimizer transforms the parsed AST to produce cleaner railroad
 * diagrams.  Passes run in two phases until a fixed point is reached:
 *
 ...
 *   collapse nested Optionals, unwrap Optional(Loop(null,...))
 * - expandOptionalSequence(): expand Optional(Sequence(..., Optional(B), ...))
 *   into a flat Choice
 *
 * **Phase 2 — Loop Analysis:**
 * - analyzeLoops(): detect loop body candidates from preceding sequence elements
 * - flattenLoopChoices(): inline Choice repeat alternatives into the loop's repeat list
 * - bodyChoiceToChoiceloop(): convert Loop(Choice-body, Epsilon-repeat) to
 *   choiceloop form in optional contexts
 *
 * @see ast_optimizer.hh for the public interface
 */
...
/**
 * @brief Test structural equality of two AST subtrees.
 *
 * Recursively compares two AST nodes for identical structure and content.
 * Used to match loop body candidates against preceding sequence elements.
 *
...
/**
 * @brief Flatten nested Sequence nodes into their parents.
 *
 * When a Sequence child is itself a Sequence, its children are spliced
 * into the parent.  Recurses into all node types.
 *
...
/**
 * @brief Flatten nested Choice nodes and expand Optional-in-Choice.
 *
 * When a Choice alternative is itself a Choice, its alternatives are
 * spliced into the parent.  When a Choice alternative is an Optional,
 * it is expanded into Epsilon + the Optional's child (and if that
 * child is a Choice, its alternatives are also spliced in).
 *
 * @param n The node to process.
 * @return Number of transformations applied.
 */
...
/**
 * @brief Remove single-child Sequence/Choice wrappers and simplify Optionals.
 *
 * Transformations performed:
 * - Sequence with one child → that child
 * - Choice with one alternative → that alternative
 ...
 * @param count Pointer to a counter incremented for each transformation.
 * @return The resulting node (caller must update its pointer).
 */
...
/**
 * @brief Expand nested optionals inside sequences into flat choices.
 *
 * Transforms the pattern:
 * @code
 *   Optional(Sequence(A..., Optional(B), C...))
 * @endcode
 * into:
 * @code
...
/**
 * @brief Count trailing elements of a repeat that match the parent sequence.
 *
 * Repeats in the AST are in forward (unreversed) order.  In the railroad
 * diagram they run right-to-left, so the last element of the repeat is
 * what appears first in the feedback path.  This function compares
 * trailing elements of the repeat against elements preceding the loop
 * in the parent sequence.
 *
 * For a Sequence repeat, children are compared from the end backwards.
 * For a single node, it is compared directly (match count 0 or 1).
 *
 * @param repeat         The repeat node to check.
 * @param parentChildren The parent sequence's child list.
 * @param before         Index in parentChildren to start matching backwards from.
 * @return Number of matching trailing elements.
 */
...
/**
 * @brief Strip trailing elements from a repeat alternative.
 *
 * Removes @p minMatch elements from the end of a repeat node
 * (the matched portion that was factored into the loop body).
 * If a Sequence is reduced to one child, that child is returned
 * directly; if reduced to zero children, an EpsilonNode is returned.
 *
 * @param repeat   The repeat node to strip (may be deleted).
 * @param minMatch Number of trailing elements to remove.
...
/**
 * @brief Analyze a sequence to extract loop bodies from preceding elements.
 *
 * @param seq The sequence node to analyze.
 * @return Number of transformations applied.
 * @see analyzeLoops
 */
...
/**
 * @brief Detect and extract loop bodies from their sequence context.
 *
 * Handles two patterns:
 * - **Non-optional:** `Sequence([..., X, Loop(body=null, repeats=[...])])`
 *   where each repeat's trailing elements match X — the matched elements
 *   become the loop body and are removed from the parent sequence.
 * - **Optional:** `Sequence([..., X, Optional(Loop(body=null, ...))])`
 *   — same matching plus unwrapping the Optional wrapper.
 *
 * Standalone `Optional(Loop(null,...))` is simplified to `Loop(null,...)`
 * since both represent zero-or-more repetition.
 *
 * @param n The node to process (recurses into all composite types).
 * @return Number of transformations applied.
 */
...
/**
 * @brief Flatten Choice repeat alternatives into the loop's repeat list.
 *
 * If a loop repeat is a Choice, its alternatives are spliced directly
 * into the loop's repeat vector.  After flattening, redundant Epsilon
 * repeats are removed when non-Epsilon alternatives exist.
 *
 * @param n The node to process (recurses into all composite types).
 * @return Number of transformations applied.
 */
...
/**
 * @brief Convert Loop(Choice-body, Epsilon-repeat) to choiceloop form.
 *
 * When a loop has a Choice body and its only repeat is Epsilon, and the
 * loop appears in an optional context (inside a Choice with an Epsilon
 * alternative, or inside an Optional), the body's alternatives are moved
 * to the repeat list and the body is set to nullptr.  This produces
 * a more natural railroad diagram where each alternative appears as a
 * separate loopback path.
 *
 * @param n                 The node to process.
 * @param inOptionalContext Whether this node is in an optional context.
 * @return Number of transformations applied.
 */
